...
class FTPServer:
    ...
    def send_msg(self, msg, is_byte=False):
        ...
        self.conn.sendall(msg_bytes)  # 再发真实的内容

    def recv_msg(self, is_byte=False):
        head = self.conn.recv(4)  # 先收4个bytes，这里4个bytes里包含了报头的长度
        if not head:
            return
        head_json_len = struct.unpack('i', head)[0]  # 解出报头的长度
        head_json = json.loads(self.conn.recv(head_json_len).decode('utf-8'))  # 拿到报头
        ...
        while recv_size < data_len-1024:
            ...
            recv_size += 1024
        recv_data += self.conn.recv(data_len-recv_size)
        if is_byte:
            msg = recv_data
        else:
    ...
    def put(self, cmd):
        ...
        if get_file_size(self.user_dir, MB, file_size) > self.user_disk_size:
            self.send_msg(STATUS_CODE[302])
            return
        else:
            self.send_msg(STATUS_CODE[303])
    ...
    def cd(self, cmd):
        ...
        if os.path.isdir(cd_path):
            ...
            if now_path.startswith(self.user_dir):
                self.send_msg(STATUS_CODE[600])
                self.send_msg(now_path)
                return
            else:
                os.chdir(previous_path)
        self.send_msg(STATUS_CODE[601])
    ...
    def dir(self, cmd):
        ...
        self.send_msg((stdout+stderr).decode('gbk'))

    def mkdir(self, cmd):
        mkdir_path = os.path.join(os.getcwd(), cmd[1])
        if os.path.exists(mkdir_path):
            self.send_msg(STATUS_CODE[401])
        else:
            os.mkdir(mkdir_path)
            self.send_msg(STATUS_CODE[400])
    ...
    def rm(self, cmd):
        ...
        if os.path.isfile(rm_path):
            ...
            self.send_msg(STATUS_CODE[501])
            return
        self.send_msg(STATUS_CODE[500])
